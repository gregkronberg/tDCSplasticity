...
#FIXME make simple example with all objects in global namespace then make wrappers and test as you add complexity
...
class Input:
    ...
    def _build_poisson(self,N=1, rates=100):
        '''
        '''
        self.input_group = PoissonGroup(N=N, rates=rates*Hz)
...
class Nrn:
    ...
    def __init__(self, ):
        '''
        '''
        self.nrn = self._build_adex(N=1)
        self.input_nrn = Input().input_group 
        self.syn = self._build_syn(pre=self.input_nrn, post=self.nrn)
    ...
    def _build_adex(self, N=1):
        ...
        self.nrn = NeuronGroup(N, self.eq_nrn, threshold=self.P.p_nrn['threshold'], reset=self.eq_nrn_reset, method='euler')
    ...
    def _build_syn(self, pre, post, ):
        '''
        '''
        self.eq_syn = self.Eq.syn['ampa'] + '\n'+ self.Eq.syn['nmda'] + '\n'+ self.Eq.syn['stp'] +'\n' + self.Eq.syn['clopath'] + '\n' +self.Eq.syn['ampa_update_offline']

        self.eq_syn_on_pre = self.Eq.syn['ampa_on_pre'] + '\n' + self.Eq.syn['nmda_on_pre'] + '\n'+ self.Eq.syn['stp_on_pre'] + '\n' + self.Eq.syn['clopath_on_pre']

        self.syn = Synapses(pre, post, self.eq_syn, on_pre=self.eq_syn_on_pre, method='euler')
...
class Run:
    ...
    def __init__(self,):
        ...
        self.Nrn.P._set_init(self.Nrn.P.p_nrn, self.Nrn.P.p_syn)
        self.Nrn.syn.connect()
        for param_key, param_val in self.Nrn.P.syn_init.iteritems():
            if hasattr(self.Nrn.syn, param_key):
                setattr(self.Nrn.syn, param_key, param_val)

        for param_key, param_val in self.Nrn.P.nrn_init.iteritems():
            if hasattr(self.Nrn.nrn, param_key):
                setattr(self.Nrn.nrn, param_key, param_val)

        self.Nrn.syn.connect(condition='j==i')
        self.spikes_in=SpikeMonitor(self.Nrn.input_nrn)
        self.rec_nrn={}
        self.rec_nrn['u'] = StateMonitor(self.Nrn.nrn, ('u'), record=True)
        self.rec_syn={}
        self.rec_syn['w_clopath'] = StateMonitor(self.Nrn.syn, ('w_clopath'), record=True)
        run_time=1000*ms
        defaultclock.dt = .1*ms
        net = Network()
        net.add(self.Nrn.nrn, self.Nrn.syn, self.Nrn.input_nrn, self.rec_nrn['u'], self.rec_syn['w_clopath'])
        # net.run(run_time, report='stdout')
        figure()
        plot(self.rec_nrn['u'].t/ms, self.rec_nrn['u'].u/mV)
        figure()
        plot(self.rec_syn['w_clopath'].t/ms, self.rec_nrn['w_clopath'].w_clopath)
