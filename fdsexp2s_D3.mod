...
NEURON {
	POINT_PROCESS FDSExp2Syn_D3
	RANGE tau1, tau2, e, i
	NONSPECIFIC_CURRENT i

	RANGE g
	GLOBAL total
        RANGE f, tau_F, d1, tau_D1, d2, tau_D2, d3, tau_D3
}
...
PARAMETER {
	...
	: the (1) is needed for the range limits to be effective
        ...
        d2 = 0.975 (1) < 0, 1 >     : intermediate depression
        tau_D2 = 9200 (ms) < 1e-9, 1e9 >
        d3 = 0.99 (1) < 0, 1 >     : slow depression
        tau_D3 = 92000 (ms) < 1e-9, 1e9 >
}
...
NET_RECEIVE(weight (umho), F, D1, D2, D3, tsyn (ms)) {
INITIAL {
...
: printf("t\t t-tsyn\t F\t D1\t D2\t D3\t amp\t newF\t newD1\t newD2\t newD3\n")
}

        F = 1 + (F-1)*exp(-(t - tsyn)/tau_F)
        D1 = 1 - (1-D1)*exp(-(t - tsyn)/tau_D1)
        D2 = 1 - (1-D2)*exp(-(t - tsyn)/tau_D2)
        D3 = 1 - (1-D3)*exp(-(t - tsyn)/tau_D3)
: printf("%g\t%g\t%g\t%g\t%g\t%g\t%g", t, t-tsyn, F, D1, D2, D3, weight*F*D1*D2)
        tsyn = t

	state_discontinuity(A, A + weight*factor*F*D1*D2*D3)
	state_discontinuity(B, B + weight*factor*F*D1*D2*D3)
	total = total+weight*F*D1*D2*D3
