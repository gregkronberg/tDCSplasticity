...
class Run():
    ...
    def _setup_recording_vectors_df(self, p, geo, syns, nc):
        ...
        for i, location in enumerate(p['rec_idx']):
            ...
            for variable, variable_type, mechanism in p['rec_variables']:
                if variable=='t':
                    continue
                if 'data_'+variable not in rec:
                    rec['data_'+variable]=None
                # rec.at[i, variable]=None


                rec.at[i, 't'] = h.Vector()
                rec.loc[i,'t'].record(h._ref_t)
                ...
                if variable_type == 'range' and  mechanism in dir(geo[tree_key][sec_num](seg_loc)):
                    ...
                    # create recording vector
                    rec.at[i, 'data_'+variable] = h.Vector()
                    rec.loc[i, 'data_'+variable].record(var_rec)
                    ...
                    for path_key, path in p['p_path'].iteritems():
                        ...
                        if location in path['syn_idx']:
                            ...
                            for burst_i, burst in enumerate(nc[path_key][seg_i][mechanism]):
                                ...
                                # set recording from netcon object
                                nc[path_key][seg_i][mechanism][burst_i].record(rec.loc[i,'data_'+variable])
                                
                                # add location
                                rec.loc[i, 'location']=location
                                rec.loc[i, 'syn_type']=mechanism
    ...
    def _save_data_df(self, data, file_name): # save data
        '''
        '''
