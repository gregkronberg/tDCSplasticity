...
"""
import figsetup_exp_1a_ACS as figsetup
import numpy as np
import analysis
import glob
import pickle
...
def _ltp_final_bar(w_clopath_df=w_clopath_df, figsetup=figsetup):
    ...
    w_clopath_df = w_clopath_df.set_index(['path_1_syn_num','ac_field', 'ac_field_phase', ])
    # figdf.reset_index()
...
def _vtrace_mean(vtrace_df=vtrace_df, figsetup=figsetup):
    ...
    # figdf['fig_xmax']=60
    # vtrace_df['path'] = vtrace_df['path'].fillna('2')
    vtrace_df = vtrace_df.set_index(['tree','path_1_syn_num','ac_field', 'ac_field_phase', ])
    kwargs={'dt':1./40}
    array_funcs = []#[arrayfuncs._subtract_timeseries]
    array_func_kws = []#[{'islice':slice(399, 400), 'axis':1}]
...
def _vtrace_individual(vtrace_df=vtrace_df, figsetup=figsetup):
    ...
    trial_id = vtrace_df[(vtrace_df.n_paths==2) & (vtrace_df.path_2_syn_num==8)].trial_id.unique()[1]
