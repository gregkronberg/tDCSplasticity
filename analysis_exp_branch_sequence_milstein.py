...
# quick plot
#-------------------------------------------------------------------------
#somatic voltage traces for varying w_mean and nmda_factor
vtrace_df.leak_conductance[vtrace_df.leak_conductance.isna()]= 0.1*3.5e-5
vtrace_df = fncs._set_index(vtrace_df, ['path_1_w_mean','nmda_factor', 'leak_conductance'])
for index in vtrace_df.index.unique():
    plt.figure()
    for i, row in vtrace_df.loc[index].iterrows():
        if row.path_1_sequence_direction=='in':
            color='red'
            linestyle='solid'
        else:
            color='blue'
            linestyle='solid'
        plt.plot(row.data_v, color=color, linestyle=linestyle)
        plt.title(str(index))
