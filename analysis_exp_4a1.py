...
"""
import figsetup_exp_4a1 as figsetup
import numpy as np
import analysis
import glob
import pickle
...
def _vtrace_individual(vtrace_df=vtrace_df, figsetup=figsetup):
    ...
    trial_id = vtrace_df[(vtrace_df.n_paths==2) & (vtrace_df.path_2_syn_num==6)].trial_id.unique()[1]
...
if run_vtrace:
    ...
    array_funcs = []#[arrayfuncs._subtract_timeseries]
    array_func_kws = []#[{'islice':slice(399, 400), 'axis':1}]
    figs, ax = analysis.PlotFuncs()._trace_mean(df=spikes_df, figdf=figdf, variable='xcorr_soma',array_funcs=array_funcs, array_func_kws=array_func_kws,**kwargs)   
    spikes_df = spikes_df.reset_index()
    # save figure
    #------------
    for fig_key, fig in figs.iteritems():
...
#####################################################################
# average weight trace, weak path dendrites in paired vs unpaired
#####################################################################
runme=False
if runme:
    # plot average voltage trace at soma and dendrite
...
if runme:
    ...
    array_funcs = [arrayfuncs._flatten]#[arrayfuncs._subtract_timeseries]
    array_func_kws = [{}]#[{'islice':slice(399, 400), 'axis':1}]
    figs, ax = analysis.PlotFuncs()._hist(df=spikes_df, figdf=figdf, variable='spike_times',array_funcs=array_funcs, array_func_kws=array_func_kws,**kwargs)   
    spikes_df = spikes_df.reset_index()
    # save figure
    #------------
    for fig_key, fig in figs.iteritems():
...
#####################################################################
# spike time probability distribution (testing new hist function)
#####################################################################
runme=False
if runme:
    # plot average voltage trace at soma and dendrite
    #------------------------------------------------
    figtype='spike_time_hist_dend_test_'
    figdf=figsetup.BuildFigDF()._spike_time_hist()
    # figdf['fig_ymin']=-4
    # figdf['fig_dyticks']=1
    # figdf['fig_xmin']=0
    ...
    array_funcs = [arrayfuncs._flatten]#[arrayfuncs._subtract_timeseries]
    array_func_kws = [{}]#[{'islice':slice(399, 400), 'axis':1}]
    bins = 'auto'#arrayfuncs._get_bins
    bins_kw={'binmin':20}
