...
apical_names=['apic', 'user5', 'apical_tuft','apical_trunk','trunk','tuft','apical_dendrite']
basal_names=['basal', 'dend', 'dendrite']
...
def find_synapse_loc(synapse_or_segment, mapping_sections_to_subtree_index):
    ...
    if not isinstance(synapse_or_segment, neuron.nrn.Segment):
        # print 'synapse_or_segment',synapse_or_segment.hname()
    ...
    with push_section(synapse_or_segment.sec):
        ...
        section_num = full_sec_name.split('[')[-1].split(']')[0]
...
def copy_dendritic_mech(original_seg_to_reduced_seg,
    ...
    for reduced_seg, original_segs in reduced_seg_to_original_seg.items():
        ...
        for param_name, param_values in vals_per_mech_per_segment[reduced_seg].items():
            ################################################################
            # check that mechanism parameter values are numeric (added by GK)
            #################################################################
            if isinstance(param_values[0], float) or isinstance(param_values[0], int):
    ...
    for reduced_seg in non_mapped_segments:
        ...
        if parent_seg and not child_seg:
            ...
            for n in vals_per_mech_per_segment[parent_seg]:
                ...
                # check that mechanism parameter values are numeric (added by GK)
                #############################################################
                if isinstance(vals_per_mech_per_segment[parent_seg][n], float) or isinstance(vals_per_mech_per_segment[parent_seg][n], int):
        ...
        if not parent_seg and child_seg:
            ...
            for n in vals_per_mech_per_segment[child_seg]:
                ...
                # check that mechanism parameter values are numeric (added by GK)
                #############################################################
                if isinstance(vals_per_mech_per_segment[child_seg][n], float) or isinstance(vals_per_mech_per_segment[child_seg][n], int):
...
def gather_cell_subtrees(roots_of_subtrees, apical_names=['apic', 'user5']):
    '''
    '''
    #######################################################################
    # added argument 'apical_names' to handle different possible names for apical subtrees.  for example in some model definitions, apical subtrees are named 'user5'.  added by GK
    #########################################################################
    ...
    for i, soma_child in enumerate(roots_of_subtrees):
        ...
        if any([_name in soma_child.hname() for _name in apical_names ]):
        # if 'apic' in soma_child.hname() or 'user5' in soma_child.hname():
...
#####################################################################################
# created by gk to copy mechanisms from old cell soma to new reduced cell soma
#####################################################################################
def copy_neuron_section_mechanisms(origin, target, **kwargs):
    ''' copy mechanisms and parameter values from one section to another
    '''
    seg = origin(0.5)
    if 'copy_mechs' in kwargs:
        mechs=kwargs['copy_mechs']
    else:
        mechs = [_mech.name() for _mech in seg]
    ms={}
    for mech in mechs: 
        target.insert(mech)
        if 'copy_params'in kwargs:
            params=kwargs['copy_params']
        else:
        ...
        for param_i ,param in enumerate(params):
            if type(param_vals[param_i]) is float:
                setattr(target, param, param_vals[param_i])
...
def create_reduced_cell(soma_cable,
    ...
    # create soma
    # FIXME hack added by GK
    # soma was copied from original cell, however this seems to be destroyed after the cell is instantiated. a workaround is to just create a new soma here instead of copying from the original cell
    #----------------------------------------------------------
    soma_L = soma_cable[0]
    soma_diam = soma_cable[1]
    soma_space_const = soma_cable[2]
    soma_cm = soma_cable[3]
    soma_rm = soma_cable[4]
    soma_g_pas = 1./soma_rm
    soma_Ra = soma_cable[5]
    soma_e_pas = soma_cable[6]
    soma_electrotonic_length = soma_cable[7]
    cell.soma = h.Section(name='soma')
    cell.soma.L=soma_diam
    cell.soma.diam=soma_diam
    cell.soma.nseg=1
    cell.soma.Ra = soma_Ra
    cell.soma.cm = soma_cm
    # copy_neuron_section_mechanisms(origin=getattr(original_cell, soma_origin)[0], target=cell.soma)#, copy_mechs=['na3'], copy_params=['gbar_na3', 'ar_na3'])
...
def textify_seg_to_seg(segs):
    ...
    for k, v in segs.iteritems():
        k_sec = str(k.sec.hname())
        v_sec = str(v.sec.hname())
        k_x = str(k.x)
        v_x = str(v.x)
        # k_new = '_'.join([k_sec, k_x])
        k_new = (k_sec, k.x)
        # v_new = '_'.join([v_sec, v_x])
        v_new = (v_sec,v.x,v)
        ret[k_new] = v_new# (v_new,v)
...
def subtree_reductor(original_cell,
                     ...
                     cell_type='milstein', 
                     apical_names=['apic', 'user5', 'apical_tuft','apical_trunk','trunk','tuft'],
                     basal_names=['basal', 'dend', 'dendrite']
                     ):
    ...
    # print 'print hoc objectvars'
    # h.allobjectvars()
    model_obj_name = load_model(model_filename)

    #######################################################################
    # added cell_type argument to handle to different formats for cell definition. added by GK
    ########################################################################
    ...
    # Created argument apical_names, allowing this function to find apical subtrees with arbitrary names, e.g. 'user5' in the migliore 2005 model
    sections_to_delete, section_per_subtree_index, mapping_sections_to_subtree_index = \
        gather_cell_subtrees(roots_of_subtrees, apical_names=apical_names)
    ...
    new_cable_properties = [reduce_subtree(roots_of_subtrees[i], reduction_frequency) for i in num_of_subtrees]

    if total_segments_manual > 1:
        new_cables_nsegs = calculate_nsegs_from_manual_arg(new_cable_properties, total_segments_manual)
    else:
        new_cables_nsegs = calculate_nsegs_from_lambda(new_cable_properties)
        if total_segments_manual > 0:
            original_cell_seg_n = (sum(i.nseg for i in list(original_cell.basal)) + sum(i.nseg for i in list(original_cell.apical)))
    ...
    new_synapses_list, subtree_ind_to_q = merge_and_add_synapses(
        ...
        cell,
        reduction_frequency,
        apical_names=apical_names,
        basal_names=basal_names)
    ...
    for _axon in cell.axon:
        _axon.connect(cell.soma, 1)
    for _basal in cell.dend:
        _basal.connect(cell.soma, 1)
    cell.apic.connect(cell.soma, 0)

    # copy mechanisms from original cell axon to reduced cell soma (added by gk)
    #---------------------------------------------------------------
    copy_neuron_section_mechanisms(origin=getattr(original_cell, 'axon')[0], target=cell.soma)
    cell.soma.ena = 55.
    # delete original cell soma
    with push_section(cell.hoc_model.soma[0]):
        h.delete_section()

    if return_seg_to_seg:
        return cell, new_synapses_list, netcons_list, original_seg_to_reduced_seg_text
        # return cell, new_synapses_list, netcons_list, original_seg_to_reduced_seg
    else:
