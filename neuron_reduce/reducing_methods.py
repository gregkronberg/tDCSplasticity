...
def find_best_real_X(Z0, ZX_goal, q, L, max_depth=100):
    ...
    for _ in range(max_depth):
        ...
        # if _ > 35:
        #     print 'distance from goal:',_, abs(ZX_goal - Z_current_X_A)
...
def reduce_synapse(cell_instance,
    ...
    #########################################################################
    # added seg argument to specify the current section when calculating the transfer impedance. added by GK
    #########################################################################
    '''
    ...
    with push_section(section):
        #####################################################################
        # seg argument is used here to specify the current section when calculating the transfer impedance. added by GK
        #####################################################################
        if seg is not None:
            orig_transfer_imp = imp_obj.transfer(synapse_location.x, sec=seg.sec) * 1000000  # ohms
        else:
            orig_transfer_imp = imp_obj.transfer(synapse_location.x, sec=section) * 1000000 
